...
# settings
 ...
 CPU_LOAD_PER_IDX="13"		# Virtual Counter CPU load percentage
 CPU_LOAD_IDX="14"		# Virtual Counter CPU load double
 LAN_IFACE="eth0"		# LAN inerface to get traffic stats from
 LAN_RX_IDX="15"		# LAN received bytes
 LAN_TX_IDX="16"		# LAN transmitted bytes
 DOMO_MEM_IDX="18"		# Domo mem usage counter (kb - bar graph)
 DOMO_MEMP_IDX="20"		# domo mem usage put in  a %
...
# post data "test with other type of device"
curl -s --max-time 20 -i -H "Accept: application/json" -s "http://$DOMO_IP:$DOMO_PORT/json.htm?type=command&param=udevice&idx=17&nvalue=0&svalue=$LANTrafficRX" > /dev/null
# post data
...
# post data
curl -s --max-time 20 -i -H "Accept: application/json" -s "http://$DOMO_IP:$DOMO_PORT/json.htm?type=command&param=udevice&idx=$DOMO_MEM_IDX&nvalue=0&svalue=$DOMOMemUsagekB" > /dev/null
curl -s --max-time 20 -i -H "Accept: application/json" -s "http://$DOMO_IP:$DOMO_PORT/json.htm?type=command&param=udevice&idx=$DOMO_MEMP_IDX&nvalue=0&svalue=$DOMOMemUsagekB" > /dev/null
